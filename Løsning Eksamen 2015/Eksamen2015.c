...
main(void){
  ...
  team_x team_arr[12];    
  int i;                                              /*"i" is being used throughout this program as an variable for loop*/                                            
  double spectators_season;                           
  int boring_team=0;  
  ...
  opgave_3(match_arr, team_arr);
    ...
    for(i=0; i<=NUMBER_OF_TEAMS; i++){
      if(team_arr[i].homewin<team_arr[i].awaywin){
      printf("%s only won %d at home but won %d away\n", team_arr[i].name, team_arr[i].homewin, team_arr[i].awaywin); 
      }
    }
  opgave_4(match_arr, team_arr);
  printf("Opgave 4:\n");
    spectators_season=team_arr[10].spectators;         /*Team_arr[10] equals to FCK - the biggest team in Denmark.
                                                         To make sure, spectators_season was lower than [i]spectators*/
    for(i=0; i<=NUMBER_OF_TEAMS; i++){
      if(team_arr[i].spectators<spectators_season){
        spectators_season=team_arr[i].spectators;       
        boring_team=i;                                  /*used to save the teams number*/
      }
    }  
    printf("%s was the team who had fewest spectators throughout the season\nwith only %.3lf spectators going to there matches.\n", team_arr[boring_team].name, spectators_season);
  ...
  opgave_6(match_arr, team_arr);
    ...
    for(i=0; i<=NUMBER_OF_TEAMS; i++){
      ...
      printf("%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", team_arr[i].name, team_arr[i].games_played, (team_arr[i].homewin+team_arr[i].awaywin), team_arr[i].draws, 
                                                    (team_arr[i].homeloss+team_arr[i].awayloss), team_arr[i].goal_diff, team_arr[i].goals_scored, team_arr[i].goals_conceded, team_arr[i].total_points);
    }
  fclose(input_file);
  return 0;
}
...
void get_match_data(match_t *output){      
    ...
    while((fgets(temp, 5000, input)) != NULL){
